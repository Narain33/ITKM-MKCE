...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    if (isset($_GET['company'])) {
        $selectedCompany = mysqli_real_escape_string($db, $_GET['company']);
        $output = '<div class="row">';




        if ($selectedCompany === 'modelwise') {
            $query = "SELECT product, model, COUNT(*) as count FROM eitems   GROUP BY product, model";
        } elseif ($selectedCompany === 'overall') {
            $query = "SELECT product, COUNT(*) as count FROM eitems  GROUP BY product";
        } else {
            $query = "SELECT product, model, COUNT(*) as count FROM eitems WHERE company='$selectedCompany'  GROUP BY product, model";
        }
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $productDetails = isset($row['model']) ? $row['model'] : $productType;
            $count = $row['count'];
            $iconClass = '';
