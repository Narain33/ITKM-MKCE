...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (move_uploaded_file($_FILES['receipt']['tmp_name'], $uploadFile)) {
        ...
        $productCount = count($_POST['material_name']);
        for ($i = 0; $i < $productCount; $i++) {
            $material = $_POST['material_name'][$i];
            $quantity = $_POST['quantity'][$i];
            $brand = $_POST['brand'][$i];

            // Check if a record with the same invoice_no, material_name, quantity, and brand already exists
            $checkDuplicateSql = "SELECT COUNT(*) FROM orders WHERE invoice_no = ? AND material_name = ? AND quantity = ? AND brand = ?";
            $checkDuplicateStmt = $db->prepare($checkDuplicateSql);
            $checkDuplicateStmt->bind_param('ssss', $invoicen, $material, $quantity, $brand);
            ...
            if ($count == 0) {
                ...
                $productSql = "INSERT INTO orders (suppliern, supp_address, indent_no, indent_date, order_no, order_date, invoice_no, invoice_date, remarks, value, gst, receipt, material_name, quantity, brand) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $productStmt = $db->prepare($productSql);
                $productStmt->bind_param('sssssssssssssis', $supplier, $saddress, $indentn, $indentdate, $orderno, $orderdate, $invoicen, $invoiced, $remarks, $value, $gst, $receiptPath, $material, $quantity, $brand);
                ...
                $res = [
                    'status' => 202,
                    'message' => 'Duplicate record found, skipping insertion'
                ];
            }
        }
                                        ...
                                        <div class="row">
                                            <div class="col-md-4">
                                                <div class="mb-3">
                                                    ...
                                                    <select class="form-control btn-rounded" name="material_name[]" required>
                                            ...
                                            <div class="col-md-4">
                                                <div class="mb-3">
                                                    ...
                                                    <input min="1" class="form-control btn-rounded" type="number" name="quantity[]" required>
                                                </div>
                                            </div>
                                            <div class="col-md-4">
                                                <div class="mb-3">
                                                    <label for="brand">Brand*</label>
                                                    <input class="form-control btn-rounded" type="text" name="brand[]" required>
                                                    ...
                                                    <tbody id="table-body">
                                                        ...
                                                        if ($result->num_rows > 0) {
                                                            while ($row = $result->fetch_assoc()) {
                                                                ...
                                                                echo "<td>";
                                                                echo "<a href='allocate.php?id=" . $row["id"] . "' class='btn btn-rounded btn-sm btn-primary'>Show</a>";
                                                                echo "<button onclick='delete_order(" . $row["id"] . ")' type='button' class='btn btn-rounded btn-danger btn-sm delete-product'>Delete</button>";
                                                                echo "</td>";
                                                                echo "</tr>";
                                                                $sn++;
                                                            }
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#add-product").click(function() {
                var productSectionClone = $(".product-section:first").clone(true);
                productSectionClone.find("select, input").val("");
                $("#product-sections").append(productSectionClone);
            });

            // Remove product section
            $("#product-sections").on("click", ".remove-product", function() {
                if ($("#product-sections .product-section").length > 1) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (response.status === 200) {
                            ...
                            swal("Error!", "Failed to insert data: " + response.message, "error");
                            $("#exampleModalCenter").modal("hide");
                            $("#table-body").load(" #table-body > *");
                            $("#save-order")[0].reset();
